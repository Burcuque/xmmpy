...
cal49b = '/xdata/xcaldata/XMM/IVAN/PN_SW/ccfdev/EPN_CTI_0049.CCF'
...
#
corr49bcc = get_ltci_corr(cal49b,cc_rtime,index=1,rawy=170)
corr49b = get_ltci_corr(cal49b,rtime,index=2)
xcorr49b = get_ltci_corr(cal49b,runyear,index=2)
xcorr49b_cc = get_ltci_corr(cal49b,runyear,index=1,rawy=170)
# CalClosed
...
ax[0].plot(runyear, xcorr48,'g-',label='v48')
#ax[0].plot(runyear, xcorr49a,'g-',label='v49a')
ax[0].plot(runyear, xcorr49b,'b-',label='v49')
ax[0].plot(runyear, xcorr49b_cc,'-',color='gray',label='CC v49 (RAWY=170)')
ax[0].axhline(1.0,color='k',ls='dashed')
ax[0].grid(True)
ax[0].set_ylabel("Observed, E$_{obs}$/E$_{lab}$")
ax[0].legend()
ax[0].set_title("PN SW Energy Scale Analysis")
#
...
ax[1].errorbar(rtime, fe49b, yerr=fe49b_err,fmt='or',alpha=0.4,label='AGNs v49')
#
...
ax[1].errorbar(cc_rtime, mnK49b, yerr=mnK49berr,fmt='^-',color='gray',alpha=0.4,label='CC v49')
ax[1].set_ylim((-40.0,40.0))
...
ax[1].legend()
#
plt.savefig(f"{wdir}/pn_sw_cti49_corrections.png",dpi=100)
plt.show()
plt.close()
