...
Step 03 of PN LW processing:  extract spectra from a spatial filter with mask 
 from Michael Smith and split per pattern with PATTERN in [1,4]

Output per CCD

Using the even list with no testing CTI in folder cti49
...
#
# get the arguments
parser = argparse.ArgumentParser(description='XMM SAS step 03 for PN LW, making spectra from full CCDs, split per pattern')
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'pn_lw_step03_patterns_cti49.log'),
                    filemode='w')
#
os.environ["SAS_ODF"] = odfdir
os.environ['SAS_CCFPATH'] = '/xdata/ccf/pub'
ccffile = os.path.join(odfdir,"ccf_test.cif")
...
# keep only double events
#evsel = "(PATTERN >= 1 && PATTERN <= 4) && #XMMEA_EP"
#
...
for i in range(12):
    ...
    #
    sel1 = f"mask({mnk_mask},1,1,RAWX,RAWY) && (CCDNR == {i+1}) && #XMMEA_EP"
    #
    # split per pattern
    #
    for xpat in np.arange(1,5):
        spec_file = f"pn_{ccd}_pat{xpat:02}_spec5.fits"
        evsel = f" {sel1} && (PATTERN == {xpat})"
        command = "evselect " + \
...
#
logging.info ("pn_lw_step03_ccd_patterns cti49 done")
#
