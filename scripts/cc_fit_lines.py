...
fout = open(f"{wdir}/pn_sw_calclosed_fit_results_{test}.csv","w")
...
for i,iobs in enumerate(obsid_list):
    ...
    in_spec = f'{wdir}/{sobs}/{test}/spectrum_bin5.fits'
    if (not os.path.isfile(in_spec)):
        print (f"No spectrum found for {sobs}")
        continue
    result = fit_cal_lines(in_spec,smooth=9, savePng=True, pngName=f"{wdir}/spectral_fit/{sobs}_{test}_fitted.png")
