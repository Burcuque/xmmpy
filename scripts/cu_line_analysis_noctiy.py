...
Using the spectra from no long-term CTI processing
...
sys.path.append(home + '/GitHub/xmmpy/scripts')
...
def fit_and_plot(obsid, pattern = "pat00", smooth = 7, specDir = "", savePng=""):
    ...
    #
    # mapping to plot the CCDs as they are physically
    #
    ...
    for j in range(12):
        ccd = j + 1
        slices = f"{specDir}/pn_{ccd:02}_{pattern}_spec5.fits"
        ...
        #
        events = f"{specDir}/pn_{ccd:02}_{pattern}_events.fits"
        ...
        output += f"{obsid},{rev},{delta_time:.4f},{ontime:.2f},{ccd:02},{rawy:.1f},{nevt},{fit_res}\n"
        #
        yfitted = fit_out.eval(x=channel)
        #print (f"Fitting CCD #{ccd:02}",fit_res)
        kx = mapping[ccd][0]
        ky = mapping[ccd][1]
        axs[kx,ky].plot(channel,counts,label=f'CCD #{ccd:02}')
        axs[kx,ky].plot(channel,y,label='')
        axs[kx,ky].plot(channel,yfitted,color='red',label='')
        axs[kx,ky].axvline(8.04, color='lime',ls='dashed',linewidth=2)
        axs[kx,ky].set_xlim((7,9))
        axs[kx,ky].set_ylim((0,ymax))
        axs[kx,ky].grid(True)
        axs[kx,ky].legend()
        if (ccd == 4):
            axs[kx,ky].set_title(f"OBS_ID: {rev}_{obsid}")
        if (ccd == 9):
            axs[kx,ky].set_xlabel("Energy (keV)")
        if (ccd == 3 or ccd == 12):
            axs[kx,ky].set_ylabel("Counts")
...
# file for output
#
fout = open(f"{outputDir}/fit_results_cuka_noctiy.csv","w")
#
...
for ik,iobs in enumerate(obslist):
    ...
    if (not os.path.isfile(slices12)):
        ...
        continue
    pngFile = f"{outputDir}/spec_fit/{obsid}_cuka_fit_noctiy.png"
    outx = fit_and_plot(obsid,specDir=sdir,savePng=pngFile)
    if (outx != None):
        print (outx,file=fout)
    else:
        print (f"Problem reading ")
#
fout.close()
print ("All done")