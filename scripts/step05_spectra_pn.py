...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'proc_{}_{}_step05.log'.format(inst,expo)),
                    filemode='w')
#
...
#
#%%
evlist = '{}_{}_evlist_clean.fits'.format(inst,expo)
...
#
src_spec = "{}_{}_src_spectrum.fits".format(inst,expo)
...
print ("*** Extracting spectrum for exposure {} in the source region".format(expo))
try:
...
#
bg_spec = "{}_{}_bkg_spectrum.fits".format(inst,expo)
command = "evselect table={} withspectrumset=yes spectrumset={}".format(evlist,bg_spec) +  \
" energycolumn=PI spectralbinsize=5 withspecranges=yes specchannelmin=0 specchannelmax={}".format(spec_chan_max) +  \
" expression='{} && ((X,Y) IN {})'".format(expr1,bg_reg)
print ("*** Extracting spectrum for exposure {} in the background region".format(expo))
try:
    result = subprocess.run(command, shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    retcode=result.returncode
    if retcode < 0:
...
rmfset = f'{inst}_{expo}.rmf'
command = "rmfgen spectrumset={} rmfset={}".format(src_spec,rmfset)
print ("*** RMF generation for {} exposure {}".format(inst,expo))
try:
    result = subprocess.run(command, shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    retcode=result.returncode
    if retcode < 0:
...
arfset = f"{inst}_{expo}.arf"
command = "arfgen spectrumset={} arfset={} withrmfset=yes rmfset={}".format(src_spec,arfset,rmfset) + \
    " badpixlocation={} detmaptype=psf".format(evlist)
print ("*** ARF generation for {} exposure {}".format(inst,expo))
try:
    result = subprocess.run(command, shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    retcode=result.returncode
    if retcode < 0:
...
#
grp0spec=f"{inst}_{expo}_spectrum_grp0.fits"
command = "specgroup spectrumset={} addfilenames=yes rmfset={}".format(src_spec,rmfset) + \
    " arfset={} backgndset={} groupedset={}".format(arfset,bg_spec,grp0spec)
print ("*** Link bkg, ARF and RMF filenames to the {} exposure {} spectrum".format(inst,expo))
try:
    result = subprocess.run(command, shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    retcode=result.returncode
    if retcode < 0:
...
#
min_cts = 25
grpspec=f"{inst}_{expo}_spectrum_grp{min_cts}.fits"
command = "specgroup spectrumset={} mincounts={} oversample=3 rmfset={}".format(src_spec,min_cts,rmfset) + \
    " arfset={} backgndset={} groupedset={}".format(arfset,bg_spec,grpspec)
print ("*** Group the {} exposure {} spectrum in bins of at least {} counts".format(inst,expo,min_cts))
try:
    result = subprocess.run(command, shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    retcode=result.returncode
    if retcode < 0:
