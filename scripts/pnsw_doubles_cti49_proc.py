...
Full workflow for EPIC-PN processing an XMM observation, filtering only double events.
...
The output will be in a folder called cti49
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'pn_proc_doubles_cti49.log'.format(task)),
                    filemode='w')
#
#
...
if ((len(evfind) < 1) or newRun):
    ...
    if (status != 0):
        raise Exception
            ...
            logging.info(f"{xtask} skipped as already done")
            print(f"{xtask} skipped as already done")
#
...
inst = 'pn'
#
ev_comm = 'evselect table={} withfilteredset=Y filteredset={}_evlist_clean_cti49.fits'.format(os.path.basename(evlist),inst) +  \
