...
Version with the testing run with CTI CCF v49
...
for target in targets:
    print (f"Adding {target}: singles")
    out_tab = f'{target}/{target}_output_xspec_cti49_singles_{ver}.csv'
    ...
    if (target in control):
        ...
        symb = 'o'
    symb = 'o'
    ax.errorbar(rtime,diff,yerr=(errs),fmt=symb,color='blue',\
               markersize=msize,label=target.capitalize())
    ax2.errorbar(rev,diff,yerr=(errs),fmt=symb,color='blue',\
               markersize=msize,label=target.capitalize())
#
    print (f"Adding {target}: doubles")
    out_tab = f'{target}/{target}_output_xspec_cti49_doubles_{ver}.csv'
    if (not os.path.isfile(out_tab)):
        print (f"No CTIv49 results found for target {target}: {out_tab}")
        continue
    t = Table.read(out_tab,data_start=0,names=("obsid","rev","delta_time","submode",\
    ...
    rev = t['rev'].data
    rtime = t['delta_time'].data
    xrtime2 = np.concatenate((xrtime2,rtime))
    ff = t['full'].data
    inst = t['inst'].data
    line = t['lineE'].data
    lineErrUp = t['lineE_low'].data
    ...
    errs = [lineErrLo*1000,lineErrUp*1000.0]
    symb = 's'
    ax.errorbar(rtime,diff,yerr=(errs),fmt=symb,alpha=0.3,color='red',\
               markersize=msize,label=target.capitalize())
    ax2.errorbar(rev,diff,yerr=(errs),fmt=symb,alpha=0.3,color='red',\
               markersize=msize,label=target.capitalize())
#
