...
Filter the PN calibrated event lists for high background and generate 
separate Good Time Intervals
...
#
# get the arguments
parser = argparse.ArgumentParser(description='XMM SAS workflow step 03 GTI filtering for pn')
parser.add_argument('obsid', type=str,
                    help='The OBSID to process')
parser.add_argument('--expo', type=str, default="S003",
                    help='The exposure filter')                    
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'proc_step03_pn.log'),
                    filemode='w')
#
...
os.chdir(ppsDir)
#
# Now check how many exposures are there, if only one then ignore the expo input
#
evlist = glob.glob(f"*_EPN_*_ImagingEvts.ds")

hasEvlist = False
if (len(evlist) == 1):
    iev = os.path.basename(evlist[0])
    expo = iev.split('_')[3]
    print(f"Only one ImagingEvts found for PN, exposure {expo}")
    logging.info(f"Only one ImagingEvts found for PN, exposure {expo}")
    rate = f'rate_pn_{expo}.fits'
    hasEvlist = True
else:
    print("Found {} ImagingEvts for PN".format(len(evlist)))
    for iexp in evlist:
        xexp = iexp.split('_')[3]
        print (xexp)
    #
    for iexp in evlist:
        if (expo in iexp):
            print(f"Found ImagingEvts for PN exposure {expo}")
            logging.info(f"Found ImagingEvts for PN exposure {expo}")
            iev = os.path.basename(iexp)
            rate = f'rate_pn_{expo}.fits'
            hasEvlist = True
#
if (not hasEvlist):
    print(f"No ImagingEvts found for exposure {expo}")
    logging.error(f"No ImagingEvts found for exposure {expo}")
    raise FileNotFoundError
...
#
gti_set = f"gti_pn_{expo}.fits"
gti_command = 'tabgtigen table={}'.format(rate) + \
' expression=\'RATE<={}\' gtiset={}'.format(use_limit,gti_set)
try:
...
#
png_file = "{}/{}_pn_{}_rates_gti.png".format(ppsDir,obsid,expo)
...
#
fset = f"pn_{expo}_evlist_clean.fits"
expr = "#XMMEA_EP && gti({},TIME) && (PI>150) && (PATTERN<=4) && (FLAG==0)".format(gti_set)
inst = 'pn'
#
ev_command = 'evselect table={} withfilteredset=Y filteredset={}'.format(iev,fset) +  \
