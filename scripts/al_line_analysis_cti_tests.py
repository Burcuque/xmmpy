...
perform a fit to the Al Kalpha line (1.486 keV)
...
vers = "cti50"
#
#fout = open(f"{outputDir}/fit_results_cuka_cti49_test51.csv","w")
...
for ik,iobs in enumerate(obslist):
    ...
    for j in range(12):
        ...
        #
        (fit_out,fit_res) = fit_cu_line(channel,counts,minMax=(1,2),line_c=1.49)
        ...
        axs[kj,k].axvline(1.486, color='lime',ls='dashed',linewidth=2)
        axs[kj,k].set_xlim((1,2))
        axs[kj,k].set_ylim((0,3000))
        axs[kj,k].grid(True)
        axs[kj,k].legend()
    ...
    #plt.tight_layout()
    plt.savefig(f"{plotDir}/{obsid}_AlKa_{vers}_plot.png",dpi=100)
    #plt.show()
    plt.close()
    break
#
