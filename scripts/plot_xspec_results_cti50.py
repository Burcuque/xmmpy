...
Version with the testing run with CTI CCF v49 & 50
...
for target in targets:
    print (f"Adding {target}: singles & doubles")
    out_tab = f'{target}/{target}_output_xspec_cti50.csv'
    if (not os.path.isfile(out_tab)):
        print (f"No CTIv50 results found for target {target}: {out_tab}")
        continue
...
tcc = Table.read(f"/xdata/xcaldata/XMM/IVAN/PN_SW/CalClosed/pn_sw_calclosed_fit_results_cti50.csv")
...
line3 = tcc['mn2_cen'].data
line3Err = tcc['mn2_cen_err'].data
...
plt.savefig(f'{wdir}/pn_sw_cti50.png',dpi=100)
plt.show()
plt.close()
