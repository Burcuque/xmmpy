...
plt.savefig(f'{wdir}/linear_extrapolation_t8.png',dpi=100)
plt.show()
plt.close()
#%%    
#
...
hdu = fits.open("/home/ivaltchanov/IVAN/PN_SW/ccfdev/EPN_CTI_0049.CCF_test8")
# test4 is the previous correction with no extrapolation for the reference energy
tt7 = hdu['LTC_TIMES'].data['TIME'][0]
xx7 = hdu['LONG_TERM_CTI'].data
sel7 = np.where(xx7['MODE_ID'] == 3)[0]
xsel7 = xx7[sel7]
tcoeff37 = xsel7['T_COEFF'][2]
# now calculate the correction
...
ax.plot(tt7,corr37,'g-',label='Previous correction curve')
...
#
isort = np.argsort(rtime)
#
# force the curve to pass through (0.0,1.0), 
# need to put a weight of 1 and 0.5 to the rest
xsp = np.insert(rtime[isort],0,0.0)
ysp = np.insert(out[isort],0,1.0)
ysp_err = np.insert(ratio_err[isort],0,1.0)
i10 = np.where(xsp >= 9.8)[0]
xsp10 = xsp[i10]
ysp10 = ysp[i10]
ysp10_err = ysp_err[i10]
weights = 1.0/ysp10_err
#weights[0] = 1.0
#weights[0] = 1.0
#isp = np.where(ysp <= 0.97)[0]
#weights[isp] = 0.3
s = UnivariateSpline(xsp10, ysp10, w=weights, k=1,s=1)

result = np.zeros_like(tt)
#
it10 = np.where(tt < 4.0)[0]
result[it10] = corr2x[it10]
#
...
it10 = np.where(tt >= 13.0)[0]
result[it10] = corr37[it10]*0.999

#it10 = np.where(tt >= 11.0)[0]
...
plt.savefig(f'{wdir}/correction_with_extrapolation_t10.png',dpi=100)
plt.show()
plt.close()
#%%
#
...
hdu49.writeto("/xdata/xcaldata/XMM/IVAN/PN_SW/ccfdev/EPN_CTI_0049.CCF_test10",overwrite=True)
