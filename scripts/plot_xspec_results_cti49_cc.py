...
targets = ["ngc2992","ngc3227","ngc3783","ngc4151","ngc4593",'ngc5548','ngc7213','ngc1566']
#
# redshifts
#
...
tcc = Table.read(f"/xdata/xcaldata/XMM/IVAN/PN_SW/CalClosed/cc_output_xspec_cti49_{ver}.csv",comment="\s*#")
...
# mean diff
#
ax.text(8.5,28.0,r'MnK$\alpha$ mean $\Delta$E (eV): {:.1f} +/- {:.1f} (n={})'.format(np.mean(cc2_diff),np.std(cc2_diff),len(cc2_diff)),fontsize=14)
...
#ax.errorbar(rtime+0.2,cc3_diff,yerr=(cc3_errs),fmt='*',\
#            marker='*',markersize=msize,color='y',fillstyle='full',\
#            linestyle='none', label=r'MnK$\beta$')
#ax2.errorbar(rev+10,cc3_diff,yerr=(cc3_errs),fmt='*',\
#            marker='*',markersize=msize,color='y',fillstyle='full',\
#            linestyle='none', label=r'MnK$\beta$')
...
#plt.savefig(f'{wdir}/pn_sw_cti49_results_{ver}.png',dpi=100)
plt.show()
plt.close()
#
#%%
