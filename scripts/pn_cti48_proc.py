...
Full workflow for EPIC processing an XMM observation
...
Will use the current EPN_CTI_0048.CCF for the long-term CTI correction

The output will be in a foder called cti48
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'pn_proc_cti48.log'.format(task)),
                    filemode='w')
#
#
...
if (not os.path.isfile(gti_file)):
    ...
    raise FileNotFoundError
#
filt_evlist = f"{ppsDir}/pn_evlist_clean_cti48.fits"

expr = "#XMMEA_EP && gti({},TIME) && (PI>150)".format(gti_file)
inst = 'pn'
#
ev_command = 'evselect table={} withfilteredset=Y filteredset={}_evlist_clean_cti48.fits'.format(os.path.basename(evlist),inst) +  \
...
#
src_spec = "pn_src_spectrum_cti48.fits"
...
#
bg_spec = "pn_bkg_spectrum_cti48.fits"
...
command = "rmfgen spectrumset={} rmfset=pn_cti48.rmf".format(src_spec,inst)
...
command = "arfgen spectrumset={} arfset=pn_cti48.arf withrmfset=yes rmfset=pn_cti48.rmf".format(src_spec) + \
...
#
command = "specgroup spectrumset={} addfilenames=yes rmfset=pn_cti48.rmf".format(src_spec) + \
    " arfset=pn_cti48.arf backgndset={} groupedset=pn_spectrum_cti48_grp0.fits".format(bg_spec)
...
#
min_cts = 25
command = "specgroup spectrumset={} mincounts={} oversample=3 rmfset=pn_cti48.rmf".format(src_spec,min_cts) + \
    " arfset=pn_cti48.arf backgndset={} groupedset=pn_spectrum_cti48_grp{}.fits".format(bg_spec,min_cts)
