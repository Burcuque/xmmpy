...
vers = "cti50"
#
fout = open(f"{outputDir}/fit_results_cuka_doubles_{vers}.csv","w")
#
...
for ik,iobs in enumerate(obslist):
    ...
    for j in range(12):
        ...
        slices = f"{outputDir}/{obsid}/{vers}/pn_{ccd}_doubles_spec5.fits"
    ...
    #plt.tight_layout()
    plt.savefig(f"{plotDir}/{obsid}_CuKa_doubles_{vers}_plot.png",dpi=100)
    #plt.show()
    plt.close()
#
fout.close()
