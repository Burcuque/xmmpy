...
parser.add_argument('--newRun', default=True, action='store_true', 
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=os.path.join(ppsDir,'pn_sw_proc.log'),
                    filemode='w')
#
...
if ((len(evfind) < 1) or newRun):
    xtask = task + f" odf={odfdir} odfaccess=all"
...
# GTI filtering
#
print ("Generating lightcurve for {}".format(evlist))
rate = 'rate_pn.fits'
...
#
tpn = fits.open('rate_pn.fits')
#
medrate = np.median(tpn[1]['RATE'])
xmad = mad_std(tpn[1]['RATE'])
...
#
#%%
gti_command = 'tabgtigen table=rate_pn.fits' + \
' expression=\'RATE<={}\' gtiset=gti_pn.fits'.format(use_limit)
try:
...
command = "rmfgen spectrumset={} rmfset=pn.rmf".format(src_spec,inst)
