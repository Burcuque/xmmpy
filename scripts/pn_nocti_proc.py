...
if ((len(evfind) < 1) or newRun):
    xtask = task + f" withctilongterm=N odf={odfdir} odfaccess=all backgroundtres=N"
...
inst = 'pn'
#
ev_command = 'evselect table={} withfilteredset=Y filteredset={}_evlist_clean_nocti.fits'.format(os.path.basename(evlist),inst) +  \
...
#
src_spec = "pn_src_spectrum_nocti.fits"
...
#
bg_spec = "pn_bkg_spectrum_nocti.fits"
...
command = "rmfgen spectrumset={} rmfset=pn_nocti.rmf".format(src_spec,inst)
...
command = "arfgen spectrumset={} arfset=pn_nocti.arf withrmfset=yes rmfset=pn_nocti.rmf".format(src_spec) + \
...
#
command = "specgroup spectrumset={} addfilenames=yes rmfset=pn_nocti.rmf".format(src_spec) + \
    " arfset=pn_nocti.arf backgndset={} groupedset=pn_spectrum_nocti_grp0.fits".format(bg_spec)
...
#
min_cts = 25
command = "specgroup spectrumset={} mincounts={} oversample=3 rmfset=pn_nocti.rmf".format(src_spec,min_cts) + \
    " arfset=pn_nocti.arf backgndset={} groupedset=pn_spectrum_nocti_grp{}.fits".format(bg_spec,min_cts)
