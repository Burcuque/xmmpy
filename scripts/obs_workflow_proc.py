...
EPIC workflow for emproc and epproc processing an XMM observation
...
if (cifDo):
    ...
    try:
        ...
        if retcode < 0:
            ...
            logging.info("cifbuild returned {}".format(retcode))
    except OSError as e:
...
if (odfDo):
    ...
    try:
        ...
        if retcode < 0:
            ...
            logging.info("odfingest returned {}".format(retcode))
    except OSError as e:
...
def doProcess(task="emproc",folder=""):
    ...
    try:
        ...
        if retcode < 0:
            ...
            logging.warning("{} was terminated by signal {}".format(task, -retcode))
        else:
            print("%s returned"%task, retcode, file=sys.stderr)
            logging.info("{} returned {}".format(task, retcode))
    except OSError as e:
